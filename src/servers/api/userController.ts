...
/** nearbyPartners GET /api/user/location */
export async function nearbyPartnersUsingGet(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.nearbyPartnersUsingGETParams,
  options?: { [key: string]: any },
) {
  return service('/api/user/location', {
    method: 'GET',
    params: {
      ...params,
    },
...
export async function signUsingPost(options?: { [key: string]: any }) {
  return service('/api/user/sign', {
    method: 'POST',
    ...(options || {}),
  });
}

/** signDays GET /api/user/sign/count */
export async function signDaysUsingGet(options?: { [key: string]: any }) {
  return service('/api/user/sign/count', {
    method: 'GET',
    ...(options || {}),
  });
}
